...
**Hermes Agent** — автономный AI-агент от Nous Research. Общается с пользователем через Telegram, вызывает инструменты, делегирует задачи. Работает как оркестратор B2B-пайплайна.

**Consilium** — собственный LLM-прокси (FastAPI). Принимает запросы от Hermes, выбирает оптимальную модель среди 16 провайдеров, фильтрует system prompt, считает токены, переключается при ошибках. Написан на Python специально для этой системы.

**B2B Pipeline** — автоматизированный сбор и анализ данных о производителях пищевого сырья в 9 странах СНГ. 6 специализированных агентов последовательно анализируют продукты, ищут источники, парсят каталоги и очищают данные.

### Почему Consilium а не FreeLLMAPI?

FreeLLMAPI (Node.js) — отличный универсальный прокси. Но нам нужно было:
- Фильтровать system prompt — вырезать технический мусор Hermes
- Классифицировать задачи — разные модели для чата/кода/поиска
- Учитывать дневные лимиты — приоритет по RPD провайдеров
- Внедрять AGENTS.md — протокол вызова слоёв всегда в модели

Всё это написано на Python, работает на VIM4 с 8GB RAM.
...
# Ключи
cp .env.example .env
# Добавить ключи провайдеров: GROQ_API_KEY_1=gsk_...
...
## Статус (21.07.2026)
- ✅ P0 исправления применены (двойной вызов, fallback, таймауты)
- ✅ Rate Limiter отключён для отладки
- ⚠️ Hermes v0.19: поток запросов работает, ответ требует отладки
- 🔄 В процессе: балльная система, Cloudflare, GitHub модели


## Статус (21.07.2026 21:15)
- ✅ P0 баги исправлены (session_key, target_provider, build_chains)
- ✅ Ответы возвращаются: content: 24 chars, 1675 chars
- ✅ Alert спам отключён
- ⚠️ Hermes v0.19: ответ валидный, но Hermes не принимает — требуется анализ формата API
- 🔄 Ищем несоответствия с требованиями v0.19


## Статус (22.07.2026 13:20) — ФИНАЛЬНЫЙ
- ✅ 11 критических исправлений от 4 ИИ
- ✅ Система чистая: кэш очищен, процессы почищены
- ⚠️ Основная проблема: Hermes v0.19 не принимает ответ (200 OK → "Provider failed")
